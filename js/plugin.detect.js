/*
 ...
 [ getVersion onWindowLoaded onDetectionDone BetterIE ]
...
var PluginDetect={version:"0.8.0",name:"PluginDetect",openTag:"<",isDefined:function(b){return typeof b!="undefined"
    ...
    if($.isIE){
        ...
        }catch(e){}verFullFloat=parseFloat($.verIEfull||"0",10);
...
},codebase:{$:1,isDisabled:function(){var a=this,b=a.$;
    return b.ActiveXEnabled&&b.isIE&&b.verIE>=7?0:1
},checkGarbage:function(d){var b=this,c=b.$,a;
    if(c.isIE&&d&&c.isEnabled.objectProperty(d.firstChild)>0){a=c.getPROP(d.firstChild,"readyState");
    ...
    if(g.isEnabled.objectProperty(c.firstChild)>0){a=1
    };
    f.checkGarbage(c);
    c.innerHTML="";
    if(a){i.min=b
...
},z:0},Plugins:{quicktime:{mimeType:["video/quicktime","application/x-quicktimeplayer","image/x-macpaint","image/x-quicktime"],progID:"QuickTimeCheckObject.QuickTimeCheck.1",progID0:"QuickTime.QuickTime",classID:"clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B",codebase:{$:1,isMin:function(a){return this.$.codebase.isMin(this,a)
    ...
    if(!d.isIE){if(d.hasMimeType(h.mimeType)){g=d.OS!=3?d.findNavPlugin("QuickTime.*Plug-?in",0):null;
        ...
        if(b){h.setPluginStatus(0,0,b);
            return
        }if(!a||d.debug){a=h.codebase.search()
        }if(!a||d.debug){
            g=d.getAXO(h.progID);
...
}},java:{mimeType:["application/x-java-applet","application/x-java-vm","application/x-java-bean"],classID:"clsid:8AD9C840-044E-11D1-B3E9-00805F499D93",navigator:{$:1,a:(function(){var b,a=!0;
    ...
    }if((!f||d.debug)&&b.DTK.query().version){f=b.DTK.version
    }if((!f||d.debug)&&b.navMime.query().version){f=b.navMime.version
    }if((!f||d.debug)&&b.navPlugin.query().version){f=b.navPlugin.version
    }if(b.nonAppletDetectionOk(f)){c=f
    }if(!c||d.debug||a.VerifyTagsHas(2.2)||a.VerifyTagsHas(2.5)){e=b.lang.System.getProperty();
        if(e[0]){f=e[0];
            c=e[0];
            l=e[1]
    ...
    if(b.OTF==3&&b.installed!=-0.5&&b.installed!=0.5){b.installed=(b.NOTF.isJavaActive(1)==1||b.lang.System.getProperty()[0])?0.5:-0.5
    ...
    if((b.isIE&&(b.verIE<6||!b.ActiveXEnabled))||(b.isGecko&&b.compareNums(b.verGecko,b.formatNum("1.6"))<=0)||(b.isSafari&&b.OS==1&&(!b.verSafari||b.compareNums(b.verSafari,"5,1,0,0")<0))||b.isChrome){return 1
    }return 0
    ...
    if(!f||e.debug){g=/Java.*TM.*Platform[^\d]*(\d+)(?:[\.,_](\d*))?(?:\s*[Update]+\s*(\d*))?/i;
        if((g.test(a)||g.test(i))&&parseInt(RegExp.$1,10)>=5){f="1,"+RegExp.$1+","+(RegExp.$2?RegExp.$2:"0")+","+(RegExp.$3?RegExp.$3:"0")}}if(!f||e.debug){g=/Java[^\d]*Plug-in/i;
    ...
    return c
},HTML:[0,0,0,0],active:[0,0,0,0],DummyObjTagHTML:0,DummySpanTagHTML:0,allowed:[1,1,1,1],VerifyTagsHas:function(c){var d=this,b;
    for(b=0;
        b<d.allowed.length;
    ...
    }if(c.debug||(a.verify&&a.verify.isEnabled())){b.saveAsVerifyTagsArray([3,3,3])
    ...
    }else{if(c.AppletTag()&&c.ObjectTag()){a=1
    }}}};
        c.aA_=a
    }return c.aA_
    ...
    if(!g||!g.obj()||b[f][0]||(i.debug&&d.OTF<3)){return
    }c=g.obj(true);
    if(i.isIE){h=i.getPROP(c,"readyState")
    };
    if(!i.isIE||h==4){try{k=i.getNum(c.getVersion()+"");
        l=c.getVendor()+"";
        c.statusbar(i.winLoaded?" ":" ")
    }catch(j){};
        if(k&&i.isStrNum(k)){b[f]=[k,l]}}},isRange:function(a){return(/^[<>]/).test(a||"")?(a.charAt(0)==">"?1:-1):0
    ...
    if(e.should_Insert_Query(1)){if(k.OTF<2){k.OTF=2
        ...
        l[3]=[0,0];
        e.query(3)
    }if(!e.DummyObjTagHTML&&!e.isDisabled.ObjectTag()){e.DummyObjTagHTML=c.DOM.insert("object",[],[],g)
    }if(!e.DummySpanTagHTML){e.DummySpanTagHTML=c.DOM.insert("",[],[],g)
    }
    var h=k.NOTF;
    if(k.OTF<3&&h.shouldContinueQuery()){k.OTF=3;
        h.onIntervalQuery=c.handler(h.$$onIntervalQuery,h);
        if(!c.winLoaded){c.WLfuncs0.push([h.winOnLoadQuery,h])
        }setTimeout(h.onIntervalQuery,h.intervalLength)}
    return e.getResult()
}},NOTF:{$:1,count:0,countMax:25,intervalLength:250,shouldContinueQuery:function(){var f=this,e=f.$,c=f.$$,b=c.applet,a,d=0;
    if(e.winLoaded&&f.count>f.countMax){return 0
    }for(a=0;
         a<b.results.length;
         a++){if(b.HTML[a]){if(!e.winLoaded&&f.count>f.countMax&&e.codebase.checkGarbage(b.HTML[a].span)){d=1;
        b.HTML[a].DELETE=1
...
},isAppletActive_:function(d){var g=this,f=g.$,b=g.$$,l=b.navigator,a=b.applet,h=a.HTML[d],i,k,c=0,j;
    ...
    if(e.funcs){
        f.callArray(e.funcs)}if(f.DOM){f.DOM.onDoneEmptyDiv()
    }}},zz:0},devalvr:{mimeType:"application/x-devalvrx",progID:"DevalVRXCtrl.DevalVRXCtrl.1",classID:"clsid:5D2CF9D0-113A-476B-986F-288B54571614",getVersion:function(){var h=this,a=null,f,c=h.$,d,g,b;
    if(!c.isIE){f=c.findNavPlugin("DevalVR");
        if(f&&f.name&&c.hasMimeType(h.mimeType)){a=f.description.split(" ")[3]
        }h.installed=a?1:-1
    }else{g=c.getAXO(h.progID);
        if(g){b=c.getPROP(c.DOM.insert("object",["classid",h.classID],["src",""],"",h).obj(),"pluginversion");
    ...
    if(f.isString(g)){g=g.replace(/\s/g,"");
        ...
        if(c.getVersionDone===null){c.getVersionDone=0;
            ...
            if(!g){l=e
                ...
                b=b||f.isChrome;
                if(!b&&f.getMimeEnabledPlugin(c.mimeType[2],a.wmpFirefox,a.avoidPlayers)){h=f.getPROP(f.DOM.insert("object",["type",c.mimeType[2],"data",""],["src",""],"",c).obj(),"versionInfo")||h
                }}}else{h=c.version
    ...
    if(d.isString(n)){n=n.replace(/\s/g,"");
        ...
        if(h&&o){c=1;
            ...
            if(h){var p=1,b=null,r=null;
                ...
                }if(p){s=d.DOM.insert("object",["type",j.mimeType[0]],["src","","autostart","false","imagestatus","false","controls","stopbutton"],"",j).obj();
